...
ogtf.assignmentcard = function(
  ...
  const card = addelem({
    ...
    classes: [
      'assignment',
      'ogtf-' + assignmentType,
      ((isarchived) ? 'ogtf-archive' : 'ogtf-current')
    ]
  });
  // put at start
  ...
  card.updatebibbuttons = function() {
    ...
    const numitems = card.bibcontentsitems
      .getElementsByClassName("bibitem").length;
    if (card.biblastchanged > card.biblastsaved) {
...
ogtf.changedetails = async function() {
  ...
  if (respObj?.success) {
    ogtf.okmessage('New details saved.');
  }
}
...
ogtf.invitenewuser = async function() {
  ...
  if (respObj?.success) {
    ogtf.okmessage('Invitation sent.');
  } else {
    ogtf.reporterror(mainmsg);
  }
}
...
ogtf.okmessage = function(okmsg) {
  ...
  if (main?.msgdiv) {
    ...
    console.log('Message without main: ' + okmsg);
  }
}
...
ogtf.showassignments = function(assignments, isarchived = false) {
  ...
  for (const assignmentType in assignmentTypes) {
    ...
    sect.newassignmentButton = addelem({
      ...
      parent: sect,
      onclick: function(e) {
        ogtf.newassignment(this.parentNode.mytype, this);
      }
    });
    ...
    for (const assignment in assignmentTypeInfo) {
      const assignmentCard = ogtf.assignmentcard(
